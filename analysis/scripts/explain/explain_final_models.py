"""Held-out permutation importance and SHAP for the two leading models."""
...
RESULTS_DIR = Path("analysis/results/tpe_model_evaluation_missing_as_zero")
DEFAULT_OUTPUT_DIR = Path("analysis/results/final_model_explainability")
...
def aggregate_shap(raw):
    return (
        raw.groupby("Gene", as_index=False)
        .agg(
...
def plot_shap(shap_rows, feature_names, output_dir):
    ...
    plt.title("Naive Bayes held-out SHAP values")
    plt.tight_layout()
    plt.savefig(output_dir / "naive_bayes_shap_beeswarm.svg")
    plt.savefig(output_dir / "naive_bayes_shap_beeswarm.png", dpi=300)
    plt.close()
...
def main(max_splits=None, skip_shap=False, output_dir=DEFAULT_OUTPUT_DIR):
    ...
    for model_name in MODELS:
        ...
        for split, (train_idx, test_idx) in enumerate(splits):
            ...
            for gene, mean, std in zip(
                X.columns, importance.importances_mean, importance.importances_std
            ):
            ...
            if model_name == "Naive Bayes" and not skip_shap:
            ...
            print(
                ...
                + ("; SHAP complete" if model_name == "Naive Bayes" and not skip_shap else "")
            )
    ...
    if shap_rows:
        ...
        shap_raw.to_csv(output_dir / "naive_bayes_shap_values.csv", index=False)
        shap_summary.to_csv(
            output_dir / "naive_bayes_shap_summary.csv", index=False
        )
        plot_shap(shap_rows, X.columns.tolist(), output_dir)
